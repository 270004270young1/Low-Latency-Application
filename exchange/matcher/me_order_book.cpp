...
namespace Exchange{
    ...
    MEOrderBook::MEOrderBook(TickerId ticker_id, Logger *logger,MatchingEngine *matching_engine):ticker_id_(ticker_id),logger_(logger),matching_engine_(matching_engine),orders_at_price_pool_(ME_MAX_PRICE_LEVELS),order_pool_(ME_MAX_ORDER_IDS){}
    ...
    auto MEOrderBook::addOrdersAtPrice(MEOrdersAtPrice *new_orders_at_price) noexcept{
        ...
        if(UNLIKELY(!best_orders_by_price)){
            ...
            new_orders_at_price->perv_entry_ = new_orders_at_price->next_entry_ = new_orders_at_price;
        }else{
            ...
            if(add_after){
                ...
                add_after = ((new_orders_at_price->side_==Side::SELL && new_orders_at_price->price_ > target->price_)||(new_orders_at_price->side_==Side::SELL && new_orders_at_price->price_ < target->price_));
            }
            ...
            if(add_after){
                ...
                if(target==best_orders_by_price){
                    target = best_orders_by_price->perv_entry_;
                }
                new_orders_at_price->perv_entry_ = target;
                target->next_entry_->perv_entry_ = new_orders_at_price;
                new_orders_at_price->next_entry_ = target->next_entry_;
                target->next_entry_ = new_orders_at_price;

            }else{

                new_orders_at_price->perv_entry_ = target->perv_entry_;
                new_orders_at_price->next_entry_ = target;
                target->perv_entry_->next_entry_ = new_orders_at_price;
                target->perv_entry_ = new_orders_at_price;
            }
    ...
    auto MEOrderBook::removeOrdersAtPrice(Side side, Price price) noexcept{
        ...
        if(UNLIKELY(orders_at_price->next_entry_==orders_at_price)){
            ...
            orders_at_price->perv_entry_->next_entry_ = orders_at_price->next_entry_;
            orders_at_price->next_entry_->perv_entry_ = orders_at_price->perv_entry_;

            if(orders_at_price==best_orders_by_price){
                (side==Side::BUY?bids_by_price_:asks_by_price_) = orders_at_price->next_entry_;
            }

            orders_at_price->perv_entry_ = orders_at_price->next_entry_ = nullptr;
    ...
    auto MEOrderBook::match(TickerId ticker_id, ClientId client_id, Side side, OrderId client_order_id, OrderId new_market_order_id, MEOrder *itr, Qty *leaves_qty) noexcept{
        ...
        *leaves_qty -= fill_qty;
        ...
        client_response_ = {ClientResponseType::FILLED,order->client_id_,ticker_id,order->client_order_id_,order->market_order_id_,order->side_,order->price_,fill_qty,order->qty_};
